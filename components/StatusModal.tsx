...
function StatusModal({navigation, route}: any) {
  const sheetRef = useRef<BottomSheet>(null);
  const [index, setIndex] = useState(0);
  const snapPoints = useMemo(() => ['50%', '90%'], []);
  const [status, setStatus] = useState<string | null>(null);
  const [search, setSearch] = useState('');
  const [filteredProducts, setFilteredProducts] = useState(data.status);
  ...
  const handleSearch = (text: any) => {
    setSearch(text);
    if (text) {
      const newData = data.status.filter(item =>
        item.label.toLowerCase().includes(text.toLowerCase()),
      );
      setFilteredProducts(newData);
    } else {
      setFilteredProducts(data.status);
    }
  };

  const handleSubmit = (status: any) => {
    navigation.goBack();
    route.params.statusData(status);
  };
      ...
      <TouchableOpacity
        ...
        <BottomSheet
          ...
          <View style={styles.header}>
            <Text style={styles.headerText}>Choose status</Text>
            <TouchableOpacity
