...
function StateModal({navigation, route}: any) {
  ...
  const [filteredProducts, setFilteredProducts] = useState(data.states);
  ...
  const handleSearch = (text: any) => {
    setSearch(text);
    if (text) {
      const newData = data.states.filter(item =>
        item.label.toLowerCase().includes(text.toLowerCase()),
      );
      setFilteredProducts(newData);
    } else {
      setFilteredProducts(data.states);
    }
  };
      ...
      <TouchableOpacity
        ...
        <BottomSheet
          ...
          <View style={styles.header}>
            <Text style={styles.headerText}>Choose state</Text>
            <TouchableOpacity
