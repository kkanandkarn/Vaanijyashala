...
function GenderModal({navigation, route}: any) {
  const sheetRef = useRef<BottomSheet>(null);
  const [index, setIndex] = useState(0);
  const snapPoints = useMemo(() => ['50%', '90%'], []);
  const [gender, setGender] = useState<string | null>(null);
  const [search, setSearch] = useState('');
  const [filteredProducts, setFilteredProducts] = useState(data.genders);
  ...
  const handleSearch = (text: any) => {
    setSearch(text);
    if (text) {
      const newData = data.genders.filter(item =>
        item.label.toLowerCase().includes(text.toLowerCase()),
      );

      setFilteredProducts(newData);
    } else {
      setFilteredProducts(data.genders);
    }
  };

  const handleSubmit = (gender: any) => {
    navigation.goBack();
    route.params.genderData(gender);
  };
