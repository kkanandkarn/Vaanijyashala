...
function UserRoleModal({navigation, route}: any) {
  const sheetRef = useRef<BottomSheet>(null);
  const [index, setIndex] = useState(0);

  const snapPoints = useMemo(() => ['50%', '90%'], []);
  const [role, setRole] = useState<string | null>('Merchant/Seller');
  const [search, setSearch] = useState('');
  const [filteredProducts, setFilteredProducts] = useState(data.roles);
  ...
  const handleSearch = (text: any) => {
    setSearch(text);
    if (text) {
      const newData = data.roles.filter(item =>
        item.label.toLowerCase().includes(text.toLowerCase()),
      );
      setFilteredProducts(newData);
    } else {
      setFilteredProducts(data.roles);
    }
  };

  const handleSubmit = (role: any) => {
    navigation.goBack();
    route.params.roleData(role);
  };
